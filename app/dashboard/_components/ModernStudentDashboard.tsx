...
export function ModernStudentDashboard({ data }: ModernStudentDashboardProps) {
  ...
  const radialData = [
    { name: "Average", value: data.averageGrade, fill: "#3b82f6" }
  ];
    ...
    <div className="max-w-7xl mx-auto space-y-12 pb-20">
      ...
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-stretch">
        <div className="lg:col-span-1 flex">
          {userLoading ? (
            <div className="w-full bg-card rounded-3xl p-8 border border-border/50 animate-pulse flex flex-col items-center space-y-4">
        ...
        <div className="lg:col-span-2 glassy rounded-[2.5rem] p-8 md:p-12 border border-border/50 shadow-xl flex flex-col justify-center relative overflow-hidden group">
          {/* Animated Background Glow */}
          <div className="absolute -right-20 -bottom-20 w-80 h-80 bg-blue-500/10 blur-[100px] group-hover:bg-blue-500/20 transition-all duration-700" />
          ...
          >
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="text-sm font-bold text-blue-600 dark:text-blue-400 uppercase tracking-widest">Student Portal</span>
            </div>
            ...
            <p className="text-muted-foreground mt-6 text-lg max-w-xl leading-relaxed">
              {t("subtitle")}. You've completed <span className="text-foreground font-bold">{data.completedAssignments} assignments</span> so far. Keep pushing!
            </p>
          </motion.div>
        </div>
      </section>
      ...
      {/* Bottom Content: Deadlines & Feedback */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs font-bold text-muted-foreground uppercase">{data.upcomingDeadlines.length} Pending</span>
          </div>
                ...
                <div key={idx} className="p-6 rounded-3xl border border-border/40 bg-slate-50/50 dark:bg-slate-900/30 space-y-4 hover:shadow-lg transition-all duration-300">
                  ...
                  {grade.feedback && (
                    <div className="relative">
                      <div className="absolute left-0 top-0 bottom-0 w-1 bg-purple-200 dark:bg-purple-900/50 rounded-full" />
